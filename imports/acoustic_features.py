...
# -----------------------------------------------------------------------------
    def compute_features(self):
        ...
        if (np.mod((num_samples - n_win_length),n_step_length) == 0):
            n_frames = int(np.floor((num_samples - n_win_length) / n_step_length))
        else:
        ...
        pncc_mel_filters = build_erb_gamma_filters(p_FFT,self.sample_rate,\
        ...
        for time_idx in range(0,num_samples-n_win_length+1,n_step_length):
            ...
            if (not self.features["pncc"].computed):
                ...
                for idx_aud in range(0,c.NUM_PNCC_FILTERS): 
                    frame_mag_spec[idx_aud] = np.sum(np.power(np.multiply(pncc_mel_filters[idx_aud,:],abs_fft),2))
                if (bSSF):
