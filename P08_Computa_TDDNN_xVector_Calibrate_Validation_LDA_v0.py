...
log_dir = c.TDDNN_SAVE_MODELS_DIR

cp_num,__ = get_min_loss_model(log_dir) # Which checkpoint to use?
n_classes = get_num_class(c.TDDNN_TRAIN_PATH)
test_frames = c.TDDNN_NUM_WIN_SIZE
model = load_model(use_cuda, log_dir, cp_num, featureDim, n_classes)

COMPUTE_XVECTOR_FOR_CALIBRATION = False
COMPUTE_XVECTOR_FOR_VALIDATION = False
COMPUTE_XVECTOR_FOR_LDA = False
COMPUTE_XVECTOR_FOR_UBM = True
...
if (COMPUTE_XVECTOR_FOR_CALIBRATION):
    ...
    enroll_train_spk(use_cuda, test_frames, model, file_list, c.XVECTOR_TDDNN_MODEL_CALIBRATION_DIR)
...
if (COMPUTE_XVECTOR_FOR_VALIDATION):
    ...
    enroll_train_spk(use_cuda, test_frames, model, file_list, c.XVECTOR_TDDNN_MODEL_VALIDATION_DIR)

# Build VALIDATION X-VECTORS
if (COMPUTE_XVECTOR_FOR_LDA):
    print("Iniciando x-vectos para LDA")
    file_list = list_contend(c.LDA_DIR, pattern = ('.p',))
    enroll_train_spk(use_cuda, test_frames, model, file_list, c.XVECTOR_TDDNN_MODEL_LDA_DIR)

if (COMPUTE_XVECTOR_FOR_UBM):
    print("Iniciando x-vectos para UBM")
    file_list = list_contend(c.UBM_DIR, pattern = ('.p',))
    enroll_train_spk(use_cuda, test_frames, model, file_list, c.XVECTOR_TDDNN_MODEL_UBM_DIR)
print('Calculados...')
