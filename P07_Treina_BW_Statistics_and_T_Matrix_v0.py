...
if (Compute_BW_stats_T_matrix):
    ...
    ComputeStatsTrain = not os.path.exists(c.T_MATRIX_STATS_TRAIN_FILE);
    ...
    if (ComputeStatsTrain):
        ...
        ofile = open(c.T_MATRIX_STATS_TRAIN_FILE, "wb")
        dill.dump(statsTrain, ofile)
        ofile.close()
    else:
        ofile = open(c.T_MATRIX_STATS_TRAIN_FILE, "rb")
        statsTrain = dill.load(ofile)
        ofile.close()
    
    print('Iniciando calculo da matrix T... ')
    T_matrix = train_tv_space(statsTrain, UBM, c.T_MATRIX_DIM, c.T_MATRIX_MAX_ITER, c.T_MATRIX_FILE)
